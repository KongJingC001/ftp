<template>
  <div id="file">
    <div id="title">
      <h1 id="system-name">File Management System</h1>
    </div>
    <div>
      <table id="file-table">
        <tr id="table-head">
        ...
        <tr v-for="file in fileList" :key="file.relativePath">
          <td>
            <div id="address">
              ...
              <a v-if="file.directory" class="directory" :href="getLink(file)" @click.prevent="getFileList(file.relativePath)" style="vertical-align: middle">{{getPath(file.relativePath)}}</a>
              <a v-else class="file" :href="getLink(file)" target="_blank" style="vertical-align: middle">{{getPath(file.relativePath)}}</a>
            </div>
          </td>
...
<script>
import {getFileList} from "../network/file"
...
export default {
  ...
  methods: {
    ...
    getLink(f) {
      return f.directory?"file?dir=" + f.relativePath:"download?filename=" + f.relativePath;
    },
    autoSubmit() {
